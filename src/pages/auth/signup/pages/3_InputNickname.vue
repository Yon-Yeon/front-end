<template>
  <div id="nickname-input">
    <div class="page-title">당신을 소개할 닉네임을<br>입력해주세요</div>
    <base-input
      InputTitle="닉네임"
...
export default {
  ...
  methods: {
    ...
    isValidEmail(email) {
      if (email.length >= 2) {
        ...
        this.inputValided = false;
      }

      return this.inputValided;
    },
    handleResize() {
      const satValue = parseInt(getComputedStyle(document.documentElement).getPropertyValue('--sat').trim());
      const emailInput = document.getElementById('nickname-input');
  ...
  mounted() {
    ...
    document.getElementById('nickname-input').style.height = `${window.visualViewport.height} - ${getComputedStyle(document.documentElement).getPropertyValue('--sat')}px`;
